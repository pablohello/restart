...
# --- Variant A switches ---
KEEP_AIMS_BASIS = True   # if True: do NOT apply per-atom phase/sign flips and permutations; keep original FHI-aims ordering
KEEP_HARTREE     = True   # if True: keep Hamiltonian in Hartree (no * Hartree2eV)
# --------------------------

def soc_aims_get_data(ase_object,
    ...
    site_norbits = []
    m_sort_list = {0:[0], 1:[0,1,2], 2:[0,1,2,3,4], 3:[0,1,2,3,4,5,6]}
    ...
    for atom_pair in atom_pair_list:
        ...
        for s1, s2, spin_lbl in [(0,0,'up_up'), (0,1,'up_dn'), (1,0,'dn_up'), (1,1,'dn_dn')]:
            ...
            if not KEEP_AIMS_BASIS:
                sub *= parity_i[:, None]
                sub *= parity_j[None, :]
                sub = sub[sort_i, :][:, sort_j]
            # units
            H_deeph[spin_lbl][atom_pair] = (sub * Hartree2eV) if not KEEP_HARTREE else sub
...
def cspm_aims_get_data(ase_object, aims_output_dir="asi.temp", asi_std_out_f="asi.log", output_dir="preprocessed", n_spin=1, n_kpts=1):
    os.makedirs(output_dir, exist_ok=True)
    Hartree2eV = 27.2113845
    ...
    H_k = np.zeros((n_kpts, N, N, n_spin), dtype=np.complex128)  # Дополнительное измерение для спина
    S_k = np.zeros((n_kpts, N, N), dtype=np.complex128)  # Перекрытие не зависит от спина

    # Файлы матриц
    h_files = [os.path.join(aims_output_dir, "hamiltonian_up.out")]
    if n_spin == 2:
        h_files.append(os.path.join(aims_output_dir, "hamiltonian_dn.out"))
    s_file = os.path.join(aims_output_dir, "overlap-matrix.out")

    # Проверка наличия файлов
    for h_file in h_files:
        if not os.path.exists(h_file):
            print(f"Hamiltonian file {h_file} missing in {aims_output_dir}, skipping.")
            return
    if not os.path.exists(s_file):
        print(f"Overlap file {s_file} missing in {aims_output_dir}, skipping.")
        return
    ...
    for ik in range(n_kpts):
        ...
        for ispin in range(n_spin):
            H_k[ik, :, :, ispin] = np.zeros((N, N), dtype=np.complex128)
        S_k[ik] = np.zeros((N, N), dtype=np.complex128)

        # Чтение Hamiltonian
        for ispin, h_file in enumerate(h_files):
            with open(h_file, 'r') as f:
                for line in f:
                    if line.strip():
                        row, col, value = map(float, line.split())
                        row, col = int(row) - 1, int(col) - 1  # Перевод индексов в 0-based
                        H_k[ik, row, col, ispin] = value
                        H_k[ik, col, row, ispin] = value  # Заполняем симметричную часть
    ...
    # Преобразование для каждого спина
    H_R = []
    for ispin in range(n_spin):
        H_R.append(A_k_to_A_R(H_k[:, :, :, ispin], k_list, R_list))
    S_R = A_k_to_A_R(S_k, k_list, R_list)

    # Срезка и перестановка
    H_deeph = [{} for _ in range(n_spin)]
    S_deeph = {}
    for atom_pair in atom_pair_list:
        eval_key = eval(atom_pair)
        matrix_slice_i = slice(site_norbits_cumsum[eval_key[3]-1] - site_norbits[eval_key[3]-1], site_norbits_cumsum[eval_key[3]-1])
        ...
        # Для каждого спина
        for ispin in range(n_spin):
            cur_H = H_R[ispin]["[{}, {}, {}]".format(eval_key[0], eval_key[1], eval_key[2])][matrix_slice_i, matrix_slice_j]
            parity_i = np.array(phase_table[eval_key[3]-1])
            parity_j = np.array(phase_table[eval_key[4]-1])
            ...
            H_deeph[ispin][atom_pair] = cur_H * Hartree2eV
    ...
    with h5py.File(os.path.join(output_dir, "hamiltonians.h5"), 'w') as f_h:
        for ispin in range(n_spin):
            spin_label = "up" if ispin == 0 else "dn"
            for key in H_deeph[ispin].keys():
                f_h[f"{key}_{spin_label}"] = H_deeph[ispin][key]
...
def nonespin_aims_get_data(ase_object, aims_output_dir="asi.temp", asi_std_out_f="asi.log", output_dir="preprocessed"):
    os.makedirs(output_dir, exist_ok=True)
    Hartree2eV = 27.2113845

    if ase_object.calc.asi.n_spin != 1:
        print("Use other functions!")
        exit()
    ...
    for atom_pair in atom_pair_list:
        ...
        if not KEEP_AIMS_BASIS:
            cur_H = cur_H[sort_i,:][:,sort_j]
            cur_S = cur_S[sort_i,:][:,sort_j]
        H_deeph[atom_pair] = (cur_H * Hartree2eV) if not KEEP_HARTREE else cur_H
        S_deeph[atom_pair] = cur_S
